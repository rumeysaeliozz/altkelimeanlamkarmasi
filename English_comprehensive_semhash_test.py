...
ZEMBEREK_PATH = r'C:\Users\golive\Desktop\Know-Your-Intent_turkish\zemberek-full.jar'
...
for benchmark_dataset, (oversample, synonym_extra_samples,
        ...
        ['AskUbuntu', 'Chatbot', 'WebApplication'], [(True, True, False)], [0], [0], [2.1], ["hash"]):
    ...
    print("Prev Training data samples: \n", X_train_raw, "\n\n")
   ...
   #  print("Size of Training Data: {}".format(len(X_train_raw)))
    ...
    def benchmark(clf, X_train, y_train, X_test, y_test, target_names,
        ...
        if hasattr(clf, 'coef_'):
            ...
            if print_top10 and feature_names is not None:
                ...
                if benchmark_dataset == "Chatbot":
                    for i, label in enumerate(["DepartureTime", "FindConnection"]):
                        top10 = np.argsort(clf.coef_[0])[-10:]
                        print(trim("%s: %s" % (label, " ".join([feature_names[i] for i in top10]))))
                if benchmark_dataset == "WebApplication":
                    for i, label in enumerate(["Download Video", "Change Password","None","Export Data",
                                               "Sync Accounts" ,"Filter Spam","Find Alternative","Delete Account"]):
        ...
        with open("./" + RESULT_FILE, 'a', encoding='utf8') as csvFile:
            ...
            fileWriter.writerow(
                [benchmark_dataset, str(clf), str(score),
    ...
    #     fileWriter = csv.writer(csvFile, delimiter='\t')
    #     fileWriter.writerow([benchmark_dataset, str(oversample), str(synonym_extra_samples), str(augment_extra_samples),
    #                          str(additional_synonyms), str(additional_augments), str(mistake_distance),
    #                          str(preprocess_time), str(semhash_time), str(vectorize_time)])
